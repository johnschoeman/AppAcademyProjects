...
def best_value
  ...
  execute(<<-SQL)
    SELECT
      albums.title, MAX(albums.price), COUNT(tracks.*)
    FROM
      albums
      JOIN tracks
        ON tracks.album = albums.asin
    GROUP BY
      albums.title
    HAVING
      MAX(albums.price) / COUNT(tracks.*) < 0.50
  SQL
end
...
def expensive_tastes
  ...
  execute(<<-SQL)
    SELECT
      styles.style, SUM(albums_with_tracks.price)/ SUM(albums_with_tracks.track_count)
    FROM
      (SELECT
        albums.title as album_title,
        albums.asin as asin,
        albums.price as price,
        COUNT(tracks.*) as track_count
      FROM
        albums
        JOIN tracks
          ON albums.asin = tracks.album
      ...
      GROUP BY
        albums.title, albums.asin, albums.price
      ) AS albums_with_tracks
      JOIN
        styles
        ON styles.album = albums_with_tracks.asin
      GROUP BY
        styles.style
      ORDER BY
        SUM(albums_with_tracks.price)/ SUM(albums_with_tracks.track_count) DESC
      LIMIT 5

      -- why is the aggregate not AVG(albums_with_tracks.price / albums_wtih_tracks.track_count?)
