...
def craiglockhart_to_sighthill
  ...
  FROM
    (
    SELECT
      a.num AS num, a.company AS company, stopa.name AS stop_a, stopb.name AS stop_b
    FROM
      routes a
    JOIN
      routes b ON (a.company = b.company AND a.num = b.num)
  ...
  JOIN
    (
    SELECT
    d.num AS num, d.company AS company, stopc.name AS stop_c, stopd.name AS stop_d
    FROM
      routes c
    JOIN
    ...
    ) AS second_route ON (first_route.stop_b = second_route.stop_c)
  SQL
end
